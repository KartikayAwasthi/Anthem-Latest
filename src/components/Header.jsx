...
const Header = ({ activeSection, scrollToSection }) => {
  ...
  // Check if section or any of its dropdown items is active
  const isActive = (item) => {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 bg-black/70 backdrop-blur-md border-b border-white/10 shadow-lg">
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between h-20">
        ...
        {/* Desktop Navigation */}
        <nav className="hidden md:flex items-center gap-8 text-white text-sm font-medium relative">
          {navItems.map((item) => (
            <div
              key={item.name}
              ...
              <AnimatePresence>
                {hoverMenu === item.name && item.dropdown && (
                  <motion.div
                    ...
                    className="absolute top-10 left-0 w-48 bg-black/90 backdrop-blur-lg border border-white/10 rounded-lg shadow-xl p-2"
                  >
                    {item.dropdown.map((sub) => (
                      sub.isRoute ? (
                        <Link
                          key={sub.name}
                          to={sub.path}
                          className={`block px-3 py-2 rounded-md text-white hover:bg-[#ba6a5a]/20`}
